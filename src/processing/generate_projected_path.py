...
class Projector:
    ...
    def create_mesh_for_pose_index(self,pcd, trajectory_arg:List[Pose], pose_index:int, distance_path:str, robot:Robot,BATTERY_MSG:str,use_color_map:bool=False):
        ...
        for i,pose_arg in enumerate(trajectory_arg[pose_index:pose_index+horizon]):
            ...
            if pose_index+i+offset >= len(trajectory_arg):
                continue
            if pose_index+i-offset < 0:
                continue

            current_pose_index = np.where(
                (battery_msg_time >= trajectory_arg[pose_index+i-offset].timestamp) &
                (battery_msg_time < trajectory_arg[pose_index+i+offset].timestamp)) 
            ...
            deltat = trajectory_arg[pose_index+i+offset].timestamp - trajectory_arg[pose_index+i-offset].timestamp 
            total_energy = power_agv * deltat
            distance = np.linalg.norm(trajectory_arg[pose_index+i-offset].get_coordinates()-trajectory_arg[pose_index+i+offset].get_coordinates())
            ...
            if speed < self.config['min_speed']:
                print("Error: speed out of range", speed,"power_agv",power_agv)
                continue
            ...
            # create a cube with 
            data_processed.append([speed,cost_of_transport,trajectory_arg[pose_index+i].timestamp])
        ...
        if use_color_map:
            ...
            plt.scatter(np.array(data_processed[:,2]),data_processed[:,1])
            plt.show()
    ...
    def custom_draw_geometry(self, width,height,camera_intrinsic,pcd ,trajectory_arg, pose_arg, path_projected_arg,
        ...
        if len(trajectory_arg) - pose_index-5 < 0 or pose_index < 0:
            return
        ...
        if self.config['debug']:
            pcd.paint_uniform_color([1, 0, 0])
            robot_mesh = robot.get_robot_mesh()
            robot_mesh.transform(world_to_robot_body)
            colors[selected_indices] = [0,0,1]
        else:
            colors[selected_indices] = [0.9,0.9,0.9]
        pcd.colors = o3d.utility.Vector3dVector(colors)
        pcd.transform(world_to_robot_body)
        
        # if self.config['debug'] and self.config['debug_index'] ==  pose_index:
        #     o3d.visualization.draw_geometries([mesh,pcd,robot_mesh])
        ...
        # # render.scene.scene.enable_light_shadow(False)
       
        # if pcd is not None:
        #     render.scene.add_geometry("pcd",pcd,material)

        # # print("intrinsic",camera_intrinsic.shape)
        # # print("extrinsic",extrinsic.shape)

        # render.setup_camera(camera_intrinsic, extrinsic, width, height)

        # image = render.render_to_image()
        # o3d.io.write_image(f"{path_projected_arg}/{pose_arg.timestamp:.6f}.jpg", image)
        ...
        plt.imsave(f"{path_projected_arg}_debug/{pose_arg.timestamp:.6f}.jpg",np.asarray(image) ,dpi = 1)
...
class GenerateProjectPath(Processor):
    ...
    def process_multicore(self, exp:Experiment, poses:List[Pose],robot:Robot):
        ...
        clear_folder(exp.projected_path+"_debug")
        # split poses in chunks
        core_process = multiprocessing.cpu_count()//2

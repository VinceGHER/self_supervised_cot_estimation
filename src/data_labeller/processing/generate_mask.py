...
def run_model_on_half_dataset(items):
    ...
    with torch.inference_mode():
        ...
        for file in (pbar := tqdm(files)):
            ...
            green_mask = image_projected < max_cost_transport * 0.8
            ...
            for mask in masks:
                ...
                if path_score > 0.1 and path_score > wall_score:
                    keep_masks.append(mask)
                    cot = np.mean(image_projected[(mask['segmentation']) & (green_mask)])
                    mask_image[mask["segmentation"]] = cot
                    continue
                if wall_score > 0.1 and wall_score > path_score:
                    keep_masks.append(mask)
                    mask_image[mask["segmentation"]] = max_cost_transport * 0.9
                    continue

            score_mask = compute_score(mask_image, green_mask)
...
def score_mask_cost_mask(mask, projected):
    ...
    return r
def compute_score(mask, projected_mask):
    green_pixel = np.sum(np.logical_and(projected_mask == True,mask > 0))
    total_green = np.sum(projected_mask == True)
    match_rate = green_pixel / total_green
    return match_rate
...
class GenerateMask(Processor):
    ...
    def process(self,exp: Experiment):
        ...
        projected_files = [os.listdir(projected_path)[100]]


        items=[
            (projected_files, projected_path, rgb_path,mask_path,seg_path,"cuda:0", self.params_sam, self.score_thresh,self.kernel_size,self.max_cost_transport),
        ]

...
class ResNet18Confidence(nn.Module):
    def __init__(self):
        ...
        # Original first layer of ResNet18: 3 input channels, 64 output channels, kernel size 7, stride 2, padding 3
        # New first layer: 4 input channels
        ...
        self.decoder_part3 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(64, 4, kernel_size=2, stride=2),  # Back to 3 channels
            nn.Sigmoid()  # Ensure output is in [0, 1]
        )

    def forward(self, input, depth):
        initial = torch.cat([input, depth], dim=1)
        x = nn.functional.interpolate(initial, size=(224, 224), mode='nearest')
        ...
        x = nn.functional.interpolate(x, size=(240, 320), mode='nearest')

        enc1 = nn.functional.interpolate(enc1, size=(240, 320), mode='nearest')
        dec1 = nn.functional.interpolate(dec1, size=(240, 320), mode='nearest')
        return (x, initial, enc1, dec1)
    

def test_ResNet18Confidence():
    model = ResNet18Confidence()
    input = torch.randn(1, 3, 240, 320)  # Example input with RGB channels
    depth = torch.randn(1, 1, 240, 320)  # Example depth input
    (init,output, enc1, dec1) = model(input, depth)
    ...
    assert output.shape == (1, 4, 240, 320)  # Ensure the output shape is correct
    assert enc1.shape ==  (1, 64, 240, 320)  # Ensure the shape of the intermediate encoder output is correct
    assert dec1.shape ==  (1, 64, 240, 320)  # Ensure the shape of the intermediate decoder output is correct
